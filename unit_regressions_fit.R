...
#Main function
UnitReg.fit <- function(z, X, n = NA, regression = "UBXII"){ 

  source("UBXII_Kw_UW_beta_functions_forApp.R")
  
  if (any(regression == c("UBXII", "BETA", "Kw", "UW", "beta"))){
    if (regression == "UBXII"){
      loglik <- l_UBXIIreg
      cdf <- cdf_UBXII
      ell_null <- l_UBXII_noreg
    ...
    if (regression == "beta"){
      loglik <- l_betareg
      cdf <- cdf_beta
      ell_null <- l_beta_noreg
      guess_noreg <- c(mean(z),1/var(z)) 
    }  
    ...
    if (regression == "BETA"){
      ...
      summ_BETA[,2] <- se_beta
      summ_BETA[,3] <- t_value
      summ_BETA[,4] <- p_value_beta   #Se RODAR por aqui FUNCIONA.
    ...
    stop(paste(regression, "Regression not available, available regressions are \"UBXII\", ",
               "\"beta\",","\"Kw\", and \"UW\""))
  }
  
  #############################################
  #It fits a UBXII, Kw, UW or beta regression model
  #############################################
  n <- length(z)
  ### Guess ####
  ...
  if(regression == "RUBXII"||regression == "UBXII"){
  ...
  good_fit_meas_mat <- matrix(NA, nrow = 1,ncol = 4)
  colnames(good_fit_meas_mat) <- c("AIC","BIC","R-square","p-val(AD)")
  rownames(good_fit_meas_mat) <- c(regression)
  good_fit_meas_mat[1,] <- c(GAIC(2), GAIC(log(length(z))),R2_G,
...
LOOCV.unit_reg <- function(z_comp,X_comp, regression = "Kw"){
  ...
  if (any(regression == c("UBXII", "BETA", "Kw", "UW", "beta"))){
    ...
    for(c in 1:n_comp.CV){
      ...
      if (any(regression == c("UBXII", "Kw", "UW", "beta"))){
        #Fit the regression model
        mod.CV = UnitReg.fit(z_train.CV,X_train.CV,n_train.CV,regression = regression)
      ...
      mae.CV = mean((z_test.CV-z_predict.CV)^2)
...
plots_quantres <- function(quant_residuals, fitted_values, sample_size){
  ...
  hist(quant_residuals, xlab="Quantile residuals", ylab="Density", 
       xlim = c(-3,3), ylim = c(0,0.6),breaks = 5,
